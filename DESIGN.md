...
| 보호자 승인 | 기존 확인 요청·결과 화면 재사용 | `/approval/{approvalRequestId}` |
...
- 기본 전달 방법은 보호자 앱 알림이다. 쉬운 결과와 행동만 공유하고 원문은 자동 공유하지 않는다.
- 앱 알림을 누르면 해당 승인 요청으로 바로 이동한다. 전송 실패 시 보호자 앱 열기·알림 허용·문자 전송을 복구 행동으로 안내한다.
- 문자/카카오톡/전화 안내는 OS share/sms/deep link로 사용자가 직접 확정하는 복구 수단이다.
- 지원되지 않는 앱은 일반 공유 시트로 복구한다.

### Guardian approval

- 요청자, 문서 제목, 쉬운 설명, 확인된 금액·기한, 원문 근거, 승인 뒤 할 일 순서로 보여준다.
- 보호자가 승인하기 전에는 공식 납부 URL을 응답과 화면에 노출하지 않는다.
- 승인 후 `https` 공식 기관 URL이 있을 때만 `공식 납부 화면 열기`를 제공한다.
- 승인은 결제나 권한 위임이 아니며, 외부 화면에서 사용자가 마지막 결제를 직접 확정한다는 안내를 숨기지 않는다.
- 거절·만료·공유 취소 상태에는 결제 화면을 열 수 없고 가족에게 재요청하는 복구 행동을 제공한다.
